...
void FnTimeFormatter(int64_t milliseconds, int round_value, int compound,
  ...
  uint64_t total_ms =
      negative ? (uint64_t)(-milliseconds) : (uint64_t)milliseconds;
  ...
  if (compound) {
    ...
    if (days > 0)
      snprintf(result, buf_size,
               "%s%" PRIu64 "d %" PRIu64 "h %" PRIu64 "m %" PRIu64 "s", sign,
               days, hours, minutes, seconds);
    else if (hours > 0)
      snprintf(result, buf_size, "%s%" PRIu64 "h %" PRIu64 "m %" PRIu64 "s",
               sign, hours, minutes, seconds);
    else if (minutes > 0)
      snprintf(result, buf_size, "%s%" PRIu64 "m %" PRIu64 "s", sign, minutes,
               seconds);
    else if (seconds > 0)
      snprintf(result, buf_size, "%s%" PRIu64 "s", sign, seconds);
    else
      snprintf(result, buf_size, "%s%" PRIu64 "ms", sign, total_ms);
...
static PyObject *py_FnTimeFormatter(PyObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "L|pp", kwlist, &milliseconds,
                                   &round_value, &compound))
    return NULL;

  char result[128];
  FnTimeFormatter((int64_t)milliseconds, round_value, compound, result,
                  sizeof(result));
...
static struct PyModuleDef humansmodule = {
    PyModuleDef_HEAD_INIT, "humans",
    "Utility to convert time and bytes to human readable format", -1,
    HumansMethods};

PyMODINIT_FUNC PyInit_humans(void) { return PyModule_Create(&humansmodule); }

// I had to vibe code this a bit bruv i got absolutely roasted by chatgpt for
// all the flaws in my c code